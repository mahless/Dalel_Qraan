...
export const STATIONS = [
  { days: 0, name: "البداية المشرقة", emoji: "🌱" },
  { days: 15, name: "العزيمة الصادقة", emoji: "🚀" },
  { days: 30, name: "الثبات الراسخ", emoji: "⛰️" },
  { days: 45, name: "الصحبة القرآنية", emoji: "📖" },
  { days: 60, name: "القلب المتعلق", emoji: "❤️" },
  { days: 75, name: "الوارثين للفردوس", emoji: "👑" },
  { days: 90, name: "نور الدرب", emoji: "✨" },
  { days: 105, name: "أهل القرآن", emoji: "🌟" }
];

export const getStreakStation = (streak: number) => {
  const currentStationIdx = Math.floor(streak / 15);
  const currentStation = STATIONS[Math.min(currentStationIdx, STATIONS.length - 1)];
  
  const nextTarget = (currentStationIdx + 1) * 15;
  const daysRemaining = nextTarget - streak;

  return {
    ...currentStation,
    nextTarget,
    daysRemaining,
    isMaxLevel: currentStationIdx >= STATIONS.length - 1
  };
};
